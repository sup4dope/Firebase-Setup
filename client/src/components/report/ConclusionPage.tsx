          ...
          <table className="w-full table-fixed">
            <thead>
              <tr className="bg-teal-900 text-white">
                ...
                <th className="py-3 px-5 text-center font-bold tracking-tight text-sm w-[40%]">전문가 컨설팅</th>
            ...
            <tbody>
              {comparisonData.map((row, index) => (
                <tr
                  key={index}
                  className={index % 2 === 0 ? "bg-white border-b border-teal-200" : "bg-teal-900/[0.03] border-b border-teal-200"}
                >
                  <td className="py-3 px-5 text-teal-900 font-bold text-xs">{row.aspect}</td>
                  <td className="py-3 px-5">
                    <div className="flex items-center justify-center">
                      <div className="flex items-center w-full max-w-[140px] gap-2">
        ...
        <div className="relative border-2 border-red-600 bg-red-600/5 p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[#2D3748] leading-relaxed text-xs">
                정책자금 심사에서 부결될 경우, <span className="font-bold text-red-600">6개월간 동일 기관 재신청이 금지</span>됩니다. 이는 귀사의 자금 조달 일정에 심각한 차질을 줄 수 있습니다.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="relative overflow-hidden flex-1">
        ...
        <div className="relative bg-teal-900 text-white p-6 h-full flex flex-col justify-between">
          ...
          <div className="pt-6 flex justify-start items-end">
            <img src={logoGaro} alt="Logo" className="h-8 opacity-40 grayscale brightness-200 contrast-200" />
          </div>
        </div>
      </div>
