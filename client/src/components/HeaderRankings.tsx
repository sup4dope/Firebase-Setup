...
export function HeaderRankings() {
  ...
  const top3Rankings = useMemo(() => {
    ...
    const executedCustomers = customers.filter(c => {
      ...
      else if (c.status_code === '집행완료') {
        scoreDate = c.execution_date || c.contract_completion_date ||
          (c.updated_at instanceof Date ? c.updated_at.toISOString().split('T')[0] : 
           c.updated_at ? String(c.updated_at).split('T')[0] : c.entry_date);
      }
      else if (isPrepaidStatus(c.status_code || '')) {
        scoreDate = c.contract_completion_date ||
          (c.updated_at instanceof Date ? c.updated_at.toISOString().split('T')[0] : 
           c.updated_at ? String(c.updated_at).split('T')[0] : c.entry_date);
      }
      else if (isPostpaidStatus(c.status_code || '')) {
        if (c.status_code === '집행완료(후불)') {
          scoreDate = c.execution_date || c.contract_completion_date ||
            (c.updated_at instanceof Date ? c.updated_at.toISOString().split('T')[0] : 
             c.updated_at ? String(c.updated_at).split('T')[0] : c.entry_date);
        } else {
    ...
    executedCustomers.forEach(customer => {
      ...
      let effectiveStatus = customer.status_code || '';
      let executionAmount = 0;
      if (effectiveStatus === '집행완료(후불)' || effectiveStatus === '집행완료(외주)' || effectiveStatus === '집행완료') {
        executionAmount = customer.execution_amount || 0;
      } else if (isPrepaidStatus(effectiveStatus)) {
        executionAmount = effectiveStatus === '집행완료(선불)' ? (customer.execution_amount || 0) : 0;
        effectiveStatus = '계약완료(선불)';
      } else if (isPostpaidStatus(effectiveStatus)) {
        executionAmount = 0;
