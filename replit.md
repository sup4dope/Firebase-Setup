...
- **연차 관리 시스템**: 2단계 승인 워크플로우(팀장 → 총관리자) 및 전일/반차 유형 지원. 한국천문연구원 공휴일 API 연동하여 잔여 연차 계산 및 달력 인터페이스 제공. 달력 셀에 전 직원 생일(`users.ssn_front` MMDD 기반, 퇴사자 제외)을 분홍색 케이크 아이콘으로 표시(모든 역할 가시).
