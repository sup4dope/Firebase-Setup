...
- **고객 관리**: 고유 ID 체계를 사용하며, 상담부터 집행까지 7단계의 영업 퍼널(상담→쓰레기통/희망타겟→계약서발송완료→계약완료→서류취합→신청완료→집행완료)을 통해 고객 상태를 추적하고 이력 관리를 수행합니다. 전자계약 발송 시 계약유형(pre/post/out)에 따라 자동으로 `계약서발송완료(선불/후불/외주)` 상태로 이동합니다.
...
- **UTM 추적 시스템**: 랜딩페이지 유입 시 `utm_source`, `utm_medium`, `utm_campaign` 파라미터를 상담 및 고객 레코드에 저장합니다. `utm_source` 값에 따라 유입경로가 자동 매핑됩니다 (`cashnote` → 캐시노트 인앱광고, `google` → 구글애즈, `GoogleAds_Agency_Sheet` source → 구글애즈(QS), 기타/없음 → 광고). UTM 기반 유입경로도 정산 시 '광고'와 동일한 수당률이 적용됩니다.
- **외부 스프레드시트 Webhook**: `POST /api/webhook/consultation` 엔드포인트를 통해 구글 스프레드시트에서 상담 데이터를 실시간 수신합니다. 수신 즉시 Firestore `consultations` 컬렉션에 저장되고, Solapi 알림톡이 자동 발송됩니다. 동일 연락처 중복 방지 로직 포함.
- **광고통계 페이지**: `super_admin` 전용 유입경로별 광고 분석 대시보드 (`/ad-stats`). 유입→상담→쓰레기통/희망타겟→계약→집행 전환 퍼널, 쓰레기통·희망타겟 하위 카테고리별 비율, DB 등급 분류(S~D, 매출/신용/업력/사업자번호 기반) 기능을 제공합니다.
